...
void CMenu::DrawMenu() {
  ...
  if (Begin("Main", nullptr,
    ...
    PushClipRect({0, 0},
                 ...
                 false);
    RenderTwoToneBackground(H::Draw.Scale(flSideSize), F::Render.Background0,
                            F::Render.Background1, F::Render.Background2, 0.f,
                            false);
    PopClipRect();
    ...
    static int iTab = 0, iAimbotTab = 0, iVisualsTab = 0, iMiscTab = 0,
               iLogsTab = 0, iSettingsTab = 0;
    ...
    if (Section("Binds")) {
      ...
      std::function<void(int, int)> getBinds = [&](int iParent, int x) {
        for (int _iBind = 0; _iBind < F::Binds.m_vBinds.size(); _iBind++) {
          ...
          SetCursorPos(vOriginalPos);
          bool bClicked =
              Button(std::format("##{}", _iBind).c_str(), {flWidth, flHeight});
          bool bPopup = IsItemClicked(ImGuiMouseButton_Right);
          ...
          if (bClicked) {
            if (!bParent) {
              ...
              // make sure bind can't be parented to itself or any of its
              // children
              int _iBind2 = _iBind;
              Bind_t _tBind2;
...
void CMenu::DrawBinds() {
  ...
  float flWidth = flNameWidth + flInfoWidth + flStateWidth +
                  (m_bIsOpen ? H::Draw.Scale(130) : H::Draw.Scale(20));
  float flHeight = H::Draw.Scale(22 * vInfo.size() +
  ...
  if (Begin("Binds", nullptr,
    ...
    for (auto &[sName, sInfo, sState, iBind, tBind] : vInfo) {
      ...
      SetCursorPos(
          {flPosX += H::Draw.Scale(12), H::Draw.Scale(iListStart + 22 * i)});
      ...
      SetCursorPos({flPosX += flInfoWidth, H::Draw.Scale(iListStart + 22 * i)});
      FText(sState.c_str());
      PopStyleColor();

      if (m_bIsOpen) { // buttons
        SetCursorPos({flWidth - H::Draw.Scale(26),
                      H::Draw.Scale(iListStart - 2 + 22 * i)});
        bool bDelete = IconButton(ICON_MD_DELETE, H::Draw.Scale(20));

        SetCursorPos({flWidth - H::Draw.Scale(51),
                      H::Draw.Scale(iListStart - 2 + 22 * i)});
        ...
        PushTransparent(Transparent ||
                        ...
                        true);
        SetCursorPos({flWidth - H::Draw.Scale(76),
                      H::Draw.Scale(iListStart - 2 + 22 * i)});
        bool bVisibility =
        ...
        SetCursorPos({flWidth - H::Draw.Scale(101),
                      H::Draw.Scale(iListStart - 2 + 22 * i)});
