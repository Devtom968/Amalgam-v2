  ...
  try {
...

    {
      ...
      for (auto &pBase : G::Vars) {
        ...
        Save(bool, tSub) else Save(int, tSub) else Save(float, tSub) else Save(IntRange_t, tSub) else Save(
            FloatRange_t,
            tSub) else Save(std::string,
                            tSub) else Save(VA_LIST(std::
                                                        vector<std::pair<
                                                            std::string,
                                                            Color_t>>),
                                            tSub) else Save(Color_t,
                                                            tSub) else Save(Gradient_t,
                                                                            tSub) else Save(DragBox_t,
                                                                                            tSub) else Save(WindowBox_t,
                                                                                                            tSub)
      }
  ...
  try {
    ...
    if (auto tSub = tRead.get_child_optional("Vars");
      ...
      for (auto &pBase : G::Vars) {
        ...
        Load(bool, *tSub) else Load(int, *tSub) else Load(float, *tSub) else Load(IntRange_t, *tSub) else Load(
            FloatRange_t,
            *tSub) else Load(std::string,
                             *tSub) else Load(VA_LIST(std::
                                                          vector<std::pair<
                                                              std::string,
                                                              Color_t>>),
                                              *tSub) else Load(Color_t,
                                                               *tSub) else Load(Gradient_t,
                                                                                *tSub) else Load(DragBox_t,
                                                                                                 *tSub) else Load(WindowBox_t,
                                                                                                                  *tSub)
      }
    } else
      ...
      for (auto &pBase : G::Vars) {
        ...
        SaveMisc(bool, tSub) else SaveMisc(int, tSub) else SaveMisc(float, tSub) else SaveMisc(IntRange_t, tSub) else SaveMisc(
            ...
            tSub) else SaveMisc(std::string,
                                tSub) else SaveMisc(VA_LIST(std::
                                                                vector<std::pair<
                                                                    std::string,
                                                                    Color_t>>),
  ...
  try {
    ...
    for (auto &pBase : G::Vars) {
      ...
      Reset(bool) else Reset(int) else Reset(float) else Reset(IntRange_t) else Reset(FloatRange_t) else Reset(
          std::
              string) else Reset(std::
                                     vector<
                                         std::
                                             string>) else Reset(Color_t) else Reset(Gradient_t) else Reset(DragBox_t) else Reset(WindowBox_t)
    }
